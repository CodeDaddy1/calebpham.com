...
Raw footage goes into a work folder and one command runs the pipeline: it transcribes the takes locally with faster-whisper, picks the best take of each line, cuts dead space, designs the story, builds the timeline in DaVinci Resolve with cuts, transitions, brand cards and captions, and renders the finished episode. No cloud, no paid APIs. Editorial judgment runs as Claude Code subagents reading the transcript and the brief, and the numbers that gate a cut come from film studies I wrote up first.

<Figure src="/work/the-ninth-room/01-chapter-card.webp" alt="A Ninth Room chapter card rendered by the pipeline: the arch-door chapter meter with the first of five doors lit yellow, the cyan eyebrow First Stop, and the chalk title The Cockrell Butterfly Center with a butterfly emoji, on navy." width={1600} height={900} caption="A chapter card, rendered by the overlay kit and composited on the brand's navy. One yellow moment per frame; the doors are the episode's real chapter count." />

## How it is built

Python 3.9 on the interpreter macOS ships, plus ffmpeg, Pillow, and headless Chrome for the cards. There is no web service and no database: an episode is a folder of files, and every stage reads the files the stage before it wrote. The repository has 271 commits and 75 test files, run with the standard library's unittest.
...
<Figure src="/work/the-ninth-room/02-endcard.webp" alt="The Ninth Room end card: the Archway mark in chalk on navy with the channel name in Newsreader." width={1600} height={900} caption="The end card from the channel kit. The Archway is the mark; the arch radius is exactly half the width, the one true curve in the system." />

## The bar a cut must clear

The first finished episode was a transcript in shoot order: 82 beats, 78 percent of them purpose "build", one hook, one payoff, and a pace of 12.6 cuts a minute against the 18 to 22 that the channels I studied run. Nothing in the pipeline had checked. So the craft bar exists, and it is written down before the code.
...
**Measurement.** The thresholds come from three channels I studied and wrote up: 18.5, 21.6, and 18.8 cuts a minute. The shipped first cut measured 12.6. The pace estimate counts visual state changes in the plan (each beat one, each cover two) and is approximate on purpose; it compares the shape of the declared ladder against the plan, not the truth of the finished timeline.
